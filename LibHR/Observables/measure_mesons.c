    ...
    for(i=0; i<nm; i++) {
        for (t=0; t<T; t++) {
            for (x=0; x<X; x++) for (y=0; y<Y; y++) for (z=0; z<Z; z++) {
                ...
                for (int b=0;b<NF;b++){
                    for (int alpha=0;alpha<4;alpha++) for (int beta=0;beta<4;beta++){
                        _FIELD_AT(&smeared_psi[beta*nm+i],ix)->c[alpha].c[b].re = sp_smeared.c[beta].c[alpha].c[b].re;
                        _FIELD_AT(&smeared_psi[beta*nm+i],ix)->c[alpha].c[b].im = sp_smeared.c[beta].c[alpha].c[b].im;
                    }
                }
